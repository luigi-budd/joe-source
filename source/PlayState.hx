...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Obvious Skill Issue!!!!', 0.2], //From 0% to 19%
		['Skill Problem!!!', 0.4], //From 20% to 39%
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'joeStudio': 
				...
				GameOverSubstate.loopSoundName = 'gameOver-joe';
				GameOverSubstate.endSoundName = 'gameOverEnd-joe';
				GameOverSubstate.characterName = 'headsetBF dead';

				var JoeBg:BGSprite = new BGSprite('joeWeek/BACK', -281.4, -186.85, 0.9, 0.9);
				JoeBg.antialiasing = true;
                add(JoeBg);

				var JoeFloor:BGSprite = new BGSprite('joeWeek/FLOORR', -994.4, 530.55, 1.0, 0.9);
				JoeFloor.updateHitbox();
				JoeFloor.antialiasing = true;
                add(JoeFloor);

				var joeSideWalls:BGSprite = new BGSprite('joeWeek/WALL', -545.2, -399.15, 0.9, 0.9);
				joeSideWalls.updateHitbox();
				joeSideWalls.antialiasing = true;
                add(joeSideWalls);

				var JoeFancyWires:BGSprite = new BGSprite('joeWeek/WIRESS', -455, -541.85, 1.1, 1.2);
				JoeFancyWires.updateHitbox();
				JoeFancyWires.antialiasing = true;
                add(JoeFancyWires);
			...
			case 'joeStudioNoBob':
				...
				var JoeBg:BGSprite = new BGSprite('angryBass/backLOL', -281.4, -186.85, 0.9, 0.9);
				JoeBg.antialiasing = true;
                add(JoeBg);
                ...
                var bobbyDoor:BGSprite = new BGSprite('angryBass/door', -281.4, 92.75, 0.9, 0.9);
                bobbyDoor.updateHitbox();
                bobbyDoor.antialiasing = true;
                add(bobbyDoor);
				...
				JoeFancyWires.antialiasing = true;
                add(JoeFancyWires);
			case 'he sounds exactly like': //he sounds exactly like endless its scary 😲😲😨😨
				var endless:BGSprite = new BGSprite('stupid/endles', 268.45, 154.4, 1.2, 1.3);
				endless.antialiasing = true;
                add(endless);
			case 'bobbyStudio': //this has so many goddamn sprites adsajakmdalfwemmz
				GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				GameOverSubstate.loopSoundName = 'gameOver-bobby';
				GameOverSubstate.endSoundName = 'gameOverEnd-bobby';
				GameOverSubstate.characterName = 'headsetBF dead';

				var joeWall:BGSprite = new BGSprite('session2/joe back/joe wall', -23.35, -179.8, 1.2, 1.1); //joe back bg
				joeWall.antialiasing = true;
				add (joeWall);

				var joeBACKfloor:BGSprite = new BGSprite('session2/joe back/joe floor', -223.2, 512.1, 1.2, 1.2);
				joeBACKfloor.antialiasing = true;
				add (joeBACKfloor);

				var joeBackSideWalls:BGSprite = new BGSprite('session2/joe back/joe side walls', -171.3, -292.35, 1.1, 1.1);
				joeBackSideWalls.antialiasing = true;
				add (joeBackSideWalls);

				var joeWIRES:BGSprite = new BGSprite('session2/joe back/joe wires', -153.05, -294.3, 1.3, 1.2);
				joeWIRES.antialiasing = true;
				add (joeWIRES);

				var bobbyWall:BGSprite = new BGSprite('session2/bobby back', -627.2, -76.45, 0.9, 0.9); //bobby part
				bobbyWall.antialiasing = true;
				add (bobbyWall);

				var bobbyMonitor:BGSprite = new BGSprite('session2/monitor', -177.1, 259.8, 0.9, 0.9);
				bobbyMonitor.antialiasing = true;
				add (bobbyMonitor); //i cant wait to finish this and start working on bobbs songs

				var bobbyfloor:BGSprite = new BGSprite('session2/boby floor', -740.3, 745.2, 1.0, 0.9);
				bobbyfloor.antialiasing = true;
				add (bobbyfloor);

				var fuckingTable:BGSprite = new BGSprite('session2/table', -154.55, 543.8, 0.9, 0.9);
				fuckingTable.antialiasing = true;
				add (fuckingTable);

				var mixerThing:BGSprite = new BGSprite('session2/weird mixer thing', 467.8, 464.8, 0.9, 0.9);
				mixerThing.antialiasing = true;
				add (mixerThing);

				var bobbySideDoor:BGSprite = new BGSprite('session2/door wall', 1506.2, -228.2, 1.0, 0.9);
				bobbySideDoor.antialiasing = true;
				add (bobbySideDoor);

				var bobbyDumbWires:BGSprite = new BGSprite('session2/wires', -318.75, -201.2, 1.0, 1.0);
				bobbyDumbWires.antialiasing = true;
				add (bobbyDumbWires);
			case 'BETAjoeStudio':

				var oldStudioBack:BGSprite = new BGSprite('oldStudio/wall', -303.1, -32.65, .9, .9);
				oldStudioBack.antialiasing = true;
				add (oldStudioBack);

				var oldStudioFloor:BGSprite = new BGSprite('oldStudio/floor', -288.3, 782.1, .9, .9);
				oldStudioFloor.antialiasing = true;
				add (oldStudioFloor);

				var oldStudioSlider:BGSprite = new BGSprite('oldStudio/sider', -296.95, -223.5, .9, .9);
				oldStudioSlider.antialiasing = true;
				add (oldStudioSlider);

				var oldStudioWire:BGSprite = new BGSprite('oldStudio/wire', 66.2, -411.65, 1.2, .7);
				oldStudioWire.antialiasing = true;
				add (oldStudioWire);

			
			case 'tobyBattleField':
				GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				GameOverSubstate.loopSoundName = 'gameOver-joe';
				GameOverSubstate.endSoundName = 'gameOverEnd-joe';
				GameOverSubstate.characterName = 'bf';

				var tobySky:BGSprite = new BGSprite('stupid/toby/skyy', -860.55, -967.4, 1.4, .8);
				tobySky.antialiasing = true;
				add (tobySky);

				var tobyMountains:BGSprite = new BGSprite('stupid/toby/mountsss', -981.55, -638.15, 1, .6);
				tobyMountains.antialiasing = true;
				add (tobyMountains);
				
				radbird = new BGSprite('stupid/toby/watchTower', 1296.8, -206.15, 1, 1, ['week 7 watch tower']);
				add(radbird);
				
				var tobyBgGround:BGSprite = new BGSprite('stupid/toby/bg hills', -840.4, 45.2, .9, .9);
				tobyBgGround.antialiasing = true;
				add(tobyBgGround);

				var tobyGround:BGSprite = new BGSprite('stupid/toby/sandy ground', -775, 366.2, 1, 1);
				tobyGround.antialiasing = true;
				add(tobyGround);

				var tobyClouds:BGSprite = new BGSprite('stupid/toby/clouds', -1023.3, -678.35, 1.4, 1.2);
				tobyClouds.antialiasing = true;
				add(tobyClouds);
  ...
  public function clearShaderFromCamera(cam:String){
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) + (150 * iconP1.scale.x - 150) / 3 - iconOffset;
		iconP2.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - (150 * iconP2.scale.x) / 3 - iconOffset * 3;
		...
		// its not stealing if youre a programmer 😎😎
	...
	public function RecalculateRating() {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			else if (songMisses >= 10) ratingFC = "Clear (Skill Issue)";
		}
